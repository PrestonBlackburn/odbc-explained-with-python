...
def startup(conn_parameters: dict) -> socket.socket:
...
def execute(sock: socket.socket, query:str) -> str:
...
def fetch_message(sock: socket.socket) -> Generator[bytes, None, None]:
    # Message format:
    ...
    while message_length > 0:
        ...
        print("got char_tag from sock: ", char_tag.decode('utf-8'))

        length = recv_exact(sock, 4)
        payload_len = int.from_bytes(length, 'big') - 4 # 4 bytes for length
        print("got length from sock: ", payload_len)

        part = recv_exact(sock, payload_len)
        print("got part from sock: ", part)
...
def _parse_row_description(message: bytes) -> list:
    ...
    print("Row Description Raw:", message)
    idx = 2
...
def _parse_data_row(message: bytes) -> tuple:
    ...
    print("Row Raw:", message)
    idx = 2
    num_col_values = int.from_bytes(message[0:idx], 'big')
    print("Number of Column Values: ", num_col_values)
    ...
    for row_num in range(num_col_values):
        ...
        print("Row Value: ",value.decode('utf-8'))
...
def process_chunk(sock: socket.socket) -> Tuple[list, List[tuple]]:
    # https://www.postgresql.org/docs/current/protocol-message-formats.html
    ...
    message_length = 1
    message_generator = fetch_message(sock)
    ...
    while message_length > 0:
        ...
        if message_type == "Z": 
            print("End of results - ready for next query")
            return columns, rows
...
if __name__ == "__main__":
    sock = startup(parameters)
    execute(sock, """select 
        table_name,
        table_schema as schema_name, 
        table_catalog as database_name 
    from information_schema.tables limit 5;""")
    columns, rows = process_chunk(sock)
